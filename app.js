...
quickPhotoTabBtn.addEventListener('click', () => switchTab('record'));
quickSpotTabBtn.addEventListener('click', () => switchTab('spots'));
...
function updateCurrentMarker(point, accuracy = 0) {
  queueMapOp(() => {
    if (!currentLocationMarker) {
      currentLocationMarker = new kakao.maps.Marker({ position: latLng(point.lat, point.lng), title: '현재 위치' });
...
function restoreDraft() {
  ...
  if (!raw) {
    setSaveStatus(supabase ? '로컬 저장 중' : '로컬 자동저장', 'soft');
    return;
  }
  try {
...
function clearCurrentSession() {
  ...
  setSaveStatus(supabase ? '새 기록 준비' : '로컬 자동저장', 'soft');
}
...
useCurrentLocationBtn.addEventListener('click', () => {
  if (!latestPosition) {
    alert('아직 GPS 위치가 없습니다. 먼저 로그를 시작하거나 위치 권한을 허용해 주세요.');
    return;
  }
  selectedSpotCoords = { lat: latestPosition.lat, lng: latestPosition.lng };
  updateSelectedCoordsPreview();
...
window.addEventListener('online', () => { if (supabase) syncToServer(); });
